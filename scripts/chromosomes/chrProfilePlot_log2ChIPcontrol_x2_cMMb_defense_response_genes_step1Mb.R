...
# Plot smoothed windowed library-size-normalized log2(ChIP/control) coverage,
# recombination rate (cM/Mb), and feature frequency along wheat chromosomes
...
# Usage:
# ./chrProfilePlot_log2ChIPcontrol_x2_cMMb_defense_response_genes_step1Mb.R CENH3 CENH3_ChIP_SRR1686799 DMC1 DMC1_Rep1_ChIP input input_SRR6350669 input input_SRR6350669 both 1Mb 1000000 15 dodgerblue limegreen 200720 'chr3A,chr3B,chr3D'
# ./chrProfilePlot_log2ChIPcontrol_x2_cMMb_defense_response_genes_step1Mb.R H3K27me1 H3K27me1_Rep1_ChIP H3K27me3 H3K27me3_ChIP_SRR6350666 MNase MNase_Rep1 input input_SRR6350669 both 1Mb 1000000 15 navy firebrick1 200720 'chr3A,chr3B,chr3D'
# ./chrProfilePlot_log2ChIPcontrol_x2_cMMb_defense_response_genes_step1Mb.R H3K4me1 H3K4me1_Rep1_ChIP_SRR8126618 H3K27ac H3K27ac_Rep1_ChIP_SRR8126621 input input_SRR6350669 input input_SRR6350669 both 1Mb 1000000 15 magenta3 goldenrod1 200720 'chr3A,chr3B,chr3D'
...
#markControlA <- "input"
#libNameControlA <- "input_SRR6350669"
#markControlB <- "input"
#libNameControlB <- "input_SRR6350669"
#align <- "both"
#winName <- "1Mb"
#winSize <- 1000000
#N <- 15
...
centromereStart <- as.vector(read.table(paste0("chromosome_compartments/",
                                               "centromeres_outer_CENH3enriched_IWGSC_2018_Science_Table_S11_chr4AL
eftmostInterval_chr5ARightTwoIntervals.txt"))[,2])
centromereEnd <- as.vector(read.table(paste0("chromosome_compartments/",
                                             "centromeres_outer_CENH3enriched_IWGSC_2018_Science_Table_S11_chr4ALef
tmostInterval_chr5ARightTwoIntervals.txt"))[,3])
...
if(libNameControlA == "MNase_Rep1") {
  ...
  profileControlA <- read.table(paste0(covDirControlA, "MNase_Rep1_MappedOn_wheat_v1.0_lowXM_",
                                       align, "_sort_norm_binSize", winName, ".bedgraph"))
} else if(libNameControlA == "input_SRR6350669") {
  ...
  profileControlA <- read.table(paste0(covDirControlA, "input_SRR6350669_MappedOn_wheat_v1.0_lowXM_",
                                       align, "_sort_norm_binSize", winName, ".bedgraph"))
} else {
  if(!(libNameControlA %in% c("MNase_Rep1", "input_SRR6350669"))) {
    stop("libNameControlA is neither MNase_Rep1 nor input_SRR6350669")
  }
}
# Rows where the difference between end and start coordinates is > winSize
...
if(libNameControlB == "MNase_Rep1") {
  ...
  profileControlB <- read.table(paste0(covDirControlB, "MNase_Rep1_MappedOn_wheat_v1.0_lowXM_",
                                       align, "_sort_norm_binSize", winName, ".bedgraph"))
} else if(libNameControlB == "input_SRR6350669") {
  covDirControlB <- paste0("/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/",
                           "input/snakemake_ChIPseq/mapped/", align, "/bg/")
  profileControlB <- read.table(paste0(covDirControlB, "input_SRR6350669_MappedOn_wheat_v1.0_lowXM_",
                                       align, "_sort_norm_binSize", winName, ".bedgraph"))
} else {
  if(!(libNameControlB %in% c("MNase_Rep1", "input_SRR6350669"))) {
    stop("libNameControlB is neither MNase_Rep1 nor input_SRR6350669")
  }
}
# Rows where the difference between end and start coordinates is > winSize
...
# Recombination rate (cM/Mb) and feature frequency chromosome profiles
