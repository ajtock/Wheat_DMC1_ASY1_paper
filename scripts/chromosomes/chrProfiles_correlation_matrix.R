...
# log2(ChIP/input), cM/Mb, MNase, DNA methylation,
...
# Usage:
# ./chrProfiles_correlation_matrix.R 1Mb 1000000 distal 'A,B,D' smoothed
...
plotDir <- paste0("plots/correlation_matrices/")
...
profilesNew <- lapply(seq_along(profiles), function(x) {
  ...
  # cMMb, MNase, DNase and gene data sets
  ...
  # Separate DNA methylation contexts and make columns consistent as above
    ...
    DNAmethList <- list()
    # Exclude average over all 3 contexts
    for(y in 3:5) {
      print(y)
